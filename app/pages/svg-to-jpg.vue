...
useHead({
  title: t('tools.jpg.metaTitle'),
  meta: [
    { name: 'description', content: t('tools.jpg.metaDescription') },
  ],
})

const seo2Items = computed(() => {
  const raw = tm('tools.jpg.seo2Items')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    label: t(`tools.jpg.seo2Items.${i}.label`),
    desc: t(`tools.jpg.seo2Items.${i}.desc`),
  }))
})

const seo3Items = computed(() => {
  const raw = tm('tools.jpg.seo3Items')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => i)
})

const faqItems = computed(() => {
  const raw = tm('tools.jpg.faq')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    question: t(`tools.jpg.faq.${i}.q`),
    answer: t(`tools.jpg.faq.${i}.a`),
  }))
})
...
<template>
  <div>
    ...
    <!-- Converter Tool -->
    <ConverterTool format="jpg" />

    <!-- SEO Content Section -->
    <section class="py-16 px-4 bg-background">
      <div class="max-w-3xl mx-auto space-y-12">
        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.jpg.seo1Title') }}</h2>
          <p class="text-muted-foreground leading-relaxed">
            {{ t('tools.jpg.seo1Content') }}
          </p>
        </div>

        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.jpg.seo2Title') }}</h2>
        ...
        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.jpg.seo3Title') }}</h2>
          <ul class="space-y-3 text-muted-foreground">
            <li v-for="(item, i) in seo3Items" :key="i" class="flex items-start gap-3">
              <span class="w-1.5 h-1.5 rounded-full bg-primary mt-2 shrink-0" />
              <span v-html="t(`tools.jpg.seo3Items.${i}.desc`, { bold: `<strong class='text-foreground'>${t(`tools.jpg.seo3Items.${i}.bold`)}</strong>` })" />
            </li>
          </ul>
        </div>
      </div>
