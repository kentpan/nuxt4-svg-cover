...
useHead({
  title: t('tools.ico.metaTitle'),
  meta: [
    { name: 'description', content: t('tools.ico.metaDescription') },
  ],
})

const seo3Items = computed(() => {
  const raw = tm('tools.ico.seo3Items')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    label: t(`tools.ico.seo3Items.${i}.label`),
    desc: t(`tools.ico.seo3Items.${i}.desc`),
  }))
})

const faqItems = computed(() => {
  const raw = tm('tools.ico.faq')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    question: t(`tools.ico.faq.${i}.q`),
    answer: t(`tools.ico.faq.${i}.a`),
  }))
})
...
<template>
  <div>
    ...
    <!-- Converter Tool -->
    <ConverterTool format="ico" />

    <!-- SEO Content Section -->
    <section class="py-16 px-4 bg-background">
      <div class="max-w-3xl mx-auto space-y-12">
        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.ico.seo1Title') }}</h2>
          <p class="text-muted-foreground leading-relaxed">
            {{ t('tools.ico.seo1Content') }}
          </p>
        </div>

        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.ico.seo2Title') }}</h2>
          <p class="text-muted-foreground leading-relaxed">
            {{ t('tools.ico.seo2Content') }}
          </p>
        </div>

        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.ico.seo3Title') }}</h2>
