...
useHead({
  title: t('tools.webp.metaTitle'),
  meta: [
    { name: 'description', content: t('tools.webp.metaDescription') },
  ],
})

const seo2Items = computed(() => {
  const raw = tm('tools.webp.seo2Items')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    label: t(`tools.webp.seo2Items.${i}.label`),
    desc: t(`tools.webp.seo2Items.${i}.desc`),
  }))
})

const seo3Items = computed(() => {
  const raw = tm('tools.webp.seo3Items')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => i)
})

const faqItems = computed(() => {
  const raw = tm('tools.webp.faq')
  if (!Array.isArray(raw)) return []
  return Array.from({ length: raw.length }, (_, i) => ({
    question: t(`tools.webp.faq.${i}.q`),
    answer: t(`tools.webp.faq.${i}.a`),
  }))
})
...
<template>
  <div>
    ...
    <!-- Converter Tool -->
    <ConverterTool format="webp" />

    <!-- SEO Content Section -->
    <section class="py-16 px-4 bg-background">
      <div class="max-w-3xl mx-auto space-y-12">
        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.webp.seo1Title') }}</h2>
          <p class="text-muted-foreground leading-relaxed">
            {{ t('tools.webp.seo1Content') }}
          </p>
        </div>

        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.webp.seo2Title') }}</h2>
          <p class="text-muted-foreground leading-relaxed mb-4">
            {{ t('tools.webp.seo2Intro') }}
          </p>
        ...
        <div>
          <h2 class="text-2xl font-bold text-foreground mb-4">{{ t('tools.webp.seo3Title') }}</h2>
          <ul class="space-y-3 text-muted-foreground">
            <li v-for="(item, i) in seo3Items" :key="i" class="flex items-start gap-3">
              <span class="w-1.5 h-1.5 rounded-full bg-primary mt-2 shrink-0" />
              <span v-html="t(`tools.webp.seo3Items.${i}.desc`, { bold: `<strong class='text-foreground'>${t(`tools.webp.seo3Items.${i}.bold`)}</strong>` })" />
            </li>
          </ul>
        </div>
      </div>
